...
async fn health_check_works() {
    ...
    // When
    let response = client
        .get(&format!("{}/health_check", &app.application_address))
        .send()
        .await
...
async fn subscribe_returns_200_for_valid_form_data() {
    ...
    let response = client
        .post(&format!("{}/subscriptions", &app.application_address))
...
async fn spawn_app() -> TestApp {
    ...
    configuration.database.database_name = format!("newsletter-test-db{}", Uuid::new_v4());
